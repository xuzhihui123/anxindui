    ...
    <el-card class="my-card" style="margin-top: 20px" v-loading="tableisLoading" v-show="adminPower===1 ? true : false">
      <el-table
        ...
        <el-table-column
                ...
                align="center">
          <template slot-scope="scope">
            <el-button size="mini" :disabled="scope.row.orderStatus=== '审核通过' ||scope.row.orderStatus=== '审核不通过' "
                       type="success" @click="editOrder(scope.row['orderId'])"
            >
              <i class="el-icon-edit"></i>审核
            </el-button>
            <el-button size="mini" type="danger" @click="deleteOrder(scope.row['orderId'])"
