<template>
  <div class="user-list">
    ...
    <!--       头部搜索-->
    <el-form :inline="true" :model="searchUserRules" class="demo-form-inline" style="margin-top: 20px"
      ...
      <el-form-item>
        <el-select v-model="searchUserRules.isCommitOrder" placeholder="请选择报单选项">
          <el-option label="提交过报单记录" :value="0"></el-option>
          <el-option label="无提交报单记录" :value="1"></el-option>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-tag :type="scope.row.userRole === '会员' ? '' : 'success'">{{scope.row.userRole}}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
                ...
                width="180">
          <template slot-scope="scope">
            <el-tag :type="scope.row.userStatus === 'userFreeze' ? 'danger' : 'success'">{{scope.row.userStatus ===
              'userFreeze' ? '已冻结'
              : '正常'}}
            </el-tag>
          </template>
    ...
    >
      <!--      内容模块-->
      <el-form :model="editUserData" status-icon ref="editUserForm"
               label-width="100px" class="demo-ruleForm" :rules="editUserDataRules">
  ...
  export default {
    ...
    methods: {
      ...
      async getUserListByPages() {
        this.isLoading = true;
        try {
          let d = await getUserListByPage(this.userParamsPage.pageNum, this.userParamsPage.pageSize)
