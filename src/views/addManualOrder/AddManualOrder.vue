    ...
    <el-card class="my-card" style="margin-top: 20px">
      <el-form :model="addBdData" status-icon ref="addBdForm"
        ...
        <el-form-item label="用户手机号码：" prop="orderUserPhone">
          <el-input v-model="addBdData.orderUserPhone" autocomplete="off" type="number" placeholder="请输入正确的手机号码！"
                    v-loading="loadingUserPhone" :disabled="loadingUserPhone"></el-input>
        </el-form-item>
        ...
        <el-form-item label="兑换积分：" :prop="isOrderScore" v-if="isShowSdbd">
          <el-input v-model="addBdData.orderScore" autocomplete="off" type="number" placeholder="请输入兑换积分"></el-input>
        </el-form-item>

        <el-form-item label="兑换金额：" v-if="false">
          <el-input :disabled="true" v-model="addBdData.orderFaceValue" autocomplete="off" type="number"></el-input>
        </el-form-item>
        ...
        <el-form-item label="" v-show="isShowImg">
          <el-image :src="item.url" :preview-src-list="previewList" style="width:150px;margin-left:20px"
      ...
      changeFile(file, fileList) {
        ...
        if (type && (type === 'image/jpeg' || type === 'image/png' || type === 'image/jpg')) {
          ...
          fileToBase(f.raw).then(r => {
            this.isShowImg = true
            this.fileList.push({name: f.raw.name, url: r, file: f.raw, uid: f.uid})
    ...
    watch: {
        ...
        handler: (newV) => {
          ...
          that.coinList.forEach(item => {
            if (item.coinId === newV) {
              ...
              that.itemList = [{itemName: '手动报单', itemId: -1}, ...item.bankCoinItemList]
            }
          })
      ...
      'addBdData.orderScore': {
        ...
        handler: function (newV) {
          ...
          this.addBdData.orderFaceValue = Number((parseInt(newV) / 10000 * number).toFixed(2)) || null
        }
      },
      timeUsers: {
        deep: true,
        handler: function (newV) {
          if (!newV.userRole) {
            ...
            this.addBdData.orderFaceValue = Number((parseInt(this.addBdData.orderScore) / 10000 * number).toFixed(2)) || null
          }
        }
      },
      //监听图片展示
